...
class Login1 extends BaseController
{
    ...
    function builder_login_validation()
    {
     ...
     function builder_property_list(){
        ...
        $id=$_SESSION['login_id'];
        $this->global['record'] = $this->common_model->get_records('tbl_builder_register', "status='0' and id='$id' order by date_time desc")[0];
        $this->global['properties'] = $this->common_model->get_records('tbl_property_details', "status='0' and builders_info_id='$id' order by date_time desc");
            if($_SESSION['is_logged_in']==TRUE)
        {

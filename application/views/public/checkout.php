				...
				<div class="row">
					...
					<form enctype="multipart/form-data" action="" class="checkout woocommerce-checkout form_checkout row" name="checkout" this_id="form-5418972">
							...
							<div class="form-group col-md-6">
								<input type="text" required="" class="form-control" name="billing_first_name" placeholder="First name *" value="<?=$user->first_name?>">
							</div>
							<div class="form-group col-md-6">
								<input type="text" required="" class="form-control" name="billing_last_name" placeholder="Last name *" value="<?=$user->last_name?>">
							</div>
							<div class="form-group col-md-12">
								<input class="form-control" required="" type="text" name="billing_cname" placeholder="Company Name">
							</div>
							<div class="form-group col-md-12">
								<input type="text" class="form-control" name="billing_address_1" required="" placeholder="Address *">
							</div>
							<div class="form-group col-md-12">
								<input type="text" class="form-control" name="billing_address_2" required="" placeholder="Address line2">
							</div>
							<div class="form-group col-md-12">
								<input class="form-control" required="" type="text" name="billing_city" placeholder="City / Town *">
							</div>
							<div class="form-group col-md-12">
								<input class="form-control" required="" type="text" name="billing_state" placeholder="State / County *">
							</div>
							<div class="form-group col-md-12">
								<input class="form-control" required="" type="text" name="billing_postcode" placeholder="Postcode / ZIP *">
							</div>
							<div class="form-group col-md-12">
								<input class="form-control" required="" type="text" name="billing_phone" placeholder="Phone *" >
							</div>
							<div class="form-group col-md-12">
								<input class="form-control" required="" type="text" name="billing_email" placeholder="Email address *" value="<?=$user->email?>">
							</div>
							<!-- <div class="form-group col-md-12">
								<div class="chek-form">
									<div class="custome-checkbox">
							...
							<!-- <div class="form-group create-account col-md-12" style="display: none;">
								<input class="form-control" type="password" placeholder="Password" name="password">
							</div> -->
							<div class="ship_detail col-md-12 form-group">
								<div class="chek-form">
									<div class="custome-checkbox">
										<input class="form-check-input" type="checkbox" name="ship_to_different_address" id="differentaddress" value="1">
										<label class="form-check-label label_info" for="differentaddress"><span>Ship to a different address?</span></label>
									</div>
								</div>
							</div>
							<div class="different_address col-md-12" style="display: none;">
								<div class="form-group">
								...
								<div class="form-group">
									<input class="form-control"  type="text" name="shipping_cname" placeholder="Company Name">
								</div>
								<div class="form-group">
								...
								<div class="form-group">
									<input type="text" class="form-control" name="shipping_address_1"  placeholder="Address *">
								</div>
								<div class="form-group">
									<input type="text" class="form-control" name="shipping_address_2" placeholder="Address line2">
								</div>
								<div class="form-group">
								...
								<div class="form-group">
									<input class="form-control"  type="text" name="shipping_postcode" placeholder="Postcode / ZIP *">
								</div>
							</div>
							...
							<div class="table-responsive order_table">
								<table class="table">
									...
									<tbody>
										...
										<tr>
											<td><?=$record->name?> <span class="product-qty">x <?=$cart->product_quantity?></span></td>
											<td>₹<?=$record->price*$cart->product_quantity?>.00</td>
										</tr>
									<?php }?>
									</tbody>
									<tfoot>
								...
								<button type="submit" class="lab-btn"><a><span>Pay Now</span></button>
							</div>
...
<script>
function select_address(line1,line2,landmark,city,state,pincode){
    
    $("#billing_address_1").val(line1);
    $("#billing_address_2").val(line2);
    $("#billing_landmark").val(landmark);
    $("#billing_city").val(city);
    $("#billing_state").val(state);
    $("#billing_postcode").val(pincode);
}
    $("#save_address").click(function(e){
        e.preventDefault();
        var validity = 0;
        var line1 = $("#billing_address_1").val();
        var line2 = $("#billing_address_2").val();
        var landmark = $("#billing_landmark").val();
        ...
        if (city == ""){
            toastr.error("Please enter a City/Town");
            return false;
        }
        else validity++;
        
        if (state == ""){
            toastr.error("Please enter a State");
            return false;
        }
        else validity++;
    
        if( pincode == "" || !isPincode(pincode) ){
            toastr.error("Please enter a valid Postcode");
            return false;
        }
        else validity++;
        ...
        if(validity == 4) {
            $.ajax({
                type: "POST",
                url:baseURL+"save-address",
                data: {line1 : line1, line2 : line2, landmark : landmark, city : city, state: state, pincode : pincode},
                dataType: "json",
                beforeSend: function () {
        ...
        intRegex = /[0-9]+$/;
        return pin.length >= 6 && intRegex.test(pin) && pin.length <= 8;
    };
    });
     $("#payment_method_offline").focusout(function(){

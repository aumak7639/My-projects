                            ...
                            <div class="row">
                                <div class="col-md-6">                                
                                    <div class="form-group">
                                        ...
                                        <select class="form-control required" name="sub_category_id" onchange="$('#catt_id').val($('option:selected', this).attr('cat-id'))">
											<option value="">Select Sub Category</option>
											<?php foreach($sub_categories as $sub_category): ?>
												<option cat-id="<?=$sub_category->category_id?>" value="<?=$sub_category->id?>"><?=$this->common_model->get_category_name($sub_category->category_id);?> - <?=$sub_category->name?></option>
											<?php endforeach; ?>
										</select>
                  ...
                  <table class="table table-hover data_table">
					...
					<tbody>
						...
						<tr>
						  ...
						  <td><?php echo $this->common_model->get_sub_category_name($record->sub_category_id) ?></td>
						  <td><?php echo $record->name ?></td>
						  <td><?php echo $record->slug ?></td>
						  <td class="text-center">
							<?php if($record->id != 0): ?>
								<?php if(0 == 1): ?>
								<a class="btn btn-sm btn-info" data-toggle="modal" data-target="#editSlider" onclick="get_records('tbl_category_images', 'child_category_id=<?=$record->id?>')"><i class="fa fa-book"></i></a>
								
								<a class="btn btn-sm btn-info" data-toggle="modal" data-target="#addSlider" onclick="$('#addSlider input[name=child_category_id]').val('<?=$record->id?>')"><i class="fa fa-plus"></i></a>
								<?php endif; ?>
								...
								$('.update_data select[name=sub_category_id]').val('<?=$record->sub_category_id?>');
								...
								<a class="btn btn-sm btn-danger deletecats" style="<?php if($record->status == 1){ echo 'display: none;'; } ?>" href="#" data-table="tbl_child_category" data-id="<?php echo $record->id; ?>"><i class="fa fa-close"></i></a>
								
								<a class="btn btn-sm btn-success activebtn" style="<?php if($record->status == 0){ echo 'display: none;'; } ?>" href="#" data-table="tbl_child_category" data-id="<?php echo $record->id; ?>"><i class="fa fa-check"></i></a>
							<?php endif; ?>
						  </td>
							<td class="text-center">
								<?php if($record->id != 0): ?>
				...
				<div class="modal-body">
					<div class="row">
						<div class="col-md-6">                                
							<div class="form-group">
								<label for="name"> Sub Category Name <span class="text-danger">*</span></label>
								<select class="form-control required" name="sub_category_id" onchange="$('#catt_id1').val($('option:selected', this).attr('cat-id'))">
									<option value="">Select Sub Category</option>
									<?php foreach($sub_categories as $sub_category): ?>
										<option cat-id="<?=$sub_category->category_id?>" value="<?=$sub_category->id?>"><?=$sub_category->name?></option>
									<?php endforeach; ?>
								</select>
								<input type="hidden" value="tbl_child_category" name="table_name">
								<input type="hidden" value="" name="row_id">
								<input type="hidden" value="" name="category_id" id="catt_id1">
								<span class="text-danger error-span">This input is required.</span>
							</div>
						</div>
						<div class="col-md-6">                                
							...
							<div class="col-md-12">
								<div class="row">
									<div class="col-md-12">
										...
										<input type="hidden" name="child_category_id" value="0">
									</div>
								</div>
							</div>
						</div>

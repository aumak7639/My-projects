			...
			<?php $url=explode(",",$record3->video_url);?>
			   <div class="col-6">
		           ...
		           <div class="col-12">
                           ...
                           <div class="col-md-12">
				<div class="form-group">
				<?php $ids=$this->common_model->get_records('tbl_property_details', "status = '0' and post_status='A'");
				?>
				<?php $sps=explode(",",$record3->similar_properties);
				?>
					<label for="similar_properties">Similar Properties<span class="text-danger">*</span></label><br>
					<select class="chzn-select"  multiple="multiple"  name="similar_properties[]" style="width:1000px;">
					
							
				<?php foreach($ids as $id):?>
				<?php $sproperties=$this->common_model->get_records('tbl_detailed_property_info', "status = '0' and id='$id->detailed_property_info_id' and id!='$record3->id'")[0];?>
						<option value="<?=$sproperties->id?>" ><?=$sproperties->property_id?></option>
					<?php endforeach;?>
						</select><br><br>
					...
					<select class="selectpicker w100 show-tick form-control" >
								...
								<option>Selected properties will appear in this dropdown (Click to view)</option>

							<?php foreach($sps as $sp):?>

							<?php $prop_name=$this->common_model->get_records('tbl_detailed_property_info', "status = '0' and id='$sp'")[0];?>
								<option value="<?=$sp?>"  disabled><?=$prop_name->property_id?></option>
							<?php endforeach;?>
                         </select>                                       
					</div>
			</div>
